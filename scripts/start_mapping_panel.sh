...
FOLLOW_FORWARD_YAW_TRIM="$(float_param_value "${BUDDYBOT_FOLLOW_FORWARD_YAW_TRIM:--0.006}")"
...
VOICE_STRAFE_SPEED="${BUDDYBOT_VOICE_STRAFE_SPEED:-0.36}"
VOICE_DIAGONAL_SPEED="${BUDDYBOT_VOICE_DIAGONAL_SPEED:-0.42}"
VOICE_DIAGONAL_COMPONENT_SCALE="${BUDDYBOT_VOICE_DIAGONAL_COMPONENT_SCALE:-0.7071}"
VOICE_FORWARD_YAW_TRIM="${BUDDYBOT_VOICE_FORWARD_YAW_TRIM:--0.003}"
VOICE_BACKWARD_YAW_TRIM="${BUDDYBOT_VOICE_BACKWARD_YAW_TRIM:--0.018}"
VOICE_STRAFE_LEFT_YAW_TRIM="${BUDDYBOT_VOICE_STRAFE_LEFT_YAW_TRIM:-0.0}"
VOICE_STRAFE_RIGHT_YAW_TRIM="${BUDDYBOT_VOICE_STRAFE_RIGHT_YAW_TRIM:-0.0}"
VOICE_NUDGE_DURATION_SEC="${BUDDYBOT_VOICE_NUDGE_DURATION_SEC:-2.5}"
VOICE_COME_HERE_DURATION_SEC="${BUDDYBOT_VOICE_COME_HERE_DURATION_SEC:-3.0}"
...
VOICE_ZERO_BURST_COUNT="${BUDDYBOT_VOICE_ZERO_BURST_COUNT:-4}"
VOICE_SPEAKER_RATE_WPM="${BUDDYBOT_VOICE_SPEAKER_RATE_WPM:-180}"
VOICE_SPEAKER_ECHO_GUARD_SEC="${BUDDYBOT_VOICE_SPEAKER_ECHO_GUARD_SEC:-0.35}"
VOICE_SPEAK_COMMAND_RESPONSES_PARAM="$(bool_param_value "${BUDDYBOT_VOICE_SPEAK_COMMAND_RESPONSES:-0}")"
VOICE_SERVER_TTS_ENABLED_PARAM="$(bool_param_value "${BUDDYBOT_VOICE_SERVER_TTS_ENABLED:-1}")"
VOICE_SERVER_TTS_TIMEOUT_SEC="${BUDDYBOT_VOICE_SERVER_TTS_TIMEOUT_SEC:-12.0}"
VOICE_SERVER_TTS_CONNECT_TIMEOUT_SEC="${BUDDYBOT_VOICE_SERVER_TTS_CONNECT_TIMEOUT_SEC:-0.8}"
VOICE_SYSTEM_SOUND_DIR="${BUDDYBOT_VOICE_SYSTEM_SOUND_DIR:-$ROOT_DIR/software/pi5/ros2_ws/src/buddybot_voice/assets/system_sounds}"
VOICE_PIPER_MODEL_PATH="${BUDDYBOT_VOICE_PIPER_MODEL_PATH:-}"
VOICE_PIPER_MODEL_ARGS=()
if [[ -n "$VOICE_PIPER_MODEL_PATH" ]]; then
  VOICE_PIPER_MODEL_ARGS=(-p "piper_model_path:=${VOICE_PIPER_MODEL_PATH}")
fi
VOICE_AUDIO_PLAYER="${BUDDYBOT_VOICE_AUDIO_PLAYER:-auto}"
VOICE_MIC_PARAM="$(bool_param_value "$ENABLE_MIC_LISTENER")"
VOICE_SPEAKER_PARAM="$(bool_param_value "$ENABLE_PI_SPEAKER")"
COMMAND_TIMEOUT="$(float_param_value "${BUDDYBOT_COMMAND_TIMEOUT:-2.0}")"
...
echo "[mapping] voice recognition: stt_mode ${VOICE_STT_MODE}, backend ${VOICE_RECOGNITION_BACKEND}, online ${VOICE_ALLOW_ONLINE_RECOGNITION_PARAM}, language ${VOICE_RECOGNITION_LANGUAGE}, phrase ${VOICE_PHRASE_TIME_LIMIT}s moving_phrase ${VOICE_MOVING_PHRASE_TIME_LIMIT}s pause ${VOICE_PAUSE_THRESHOLD}s google_timeout ${VOICE_GOOGLE_TIMEOUT_SEC}s wake ${VOICE_WAKE_TIMEOUT}s manual_speed ${VOICE_MANUAL_SPEED} strafe_speed ${VOICE_STRAFE_SPEED} diagonal ${VOICE_DIAGONAL_SPEED}@${VOICE_DIAGONAL_COMPONENT_SCALE} yaw_trim f/b/sl/sr ${VOICE_FORWARD_YAW_TRIM}/${VOICE_BACKWARD_YAW_TRIM}/${VOICE_STRAFE_LEFT_YAW_TRIM}/${VOICE_STRAFE_RIGHT_YAW_TRIM} nudge ${VOICE_NUDGE_DURATION_SEC}s come_here ${VOICE_COME_HERE_DURATION_SEC}s continuous_max ${VOICE_CONTINUOUS_MAX_SEC}s tts_rate ${VOICE_SPEAKER_RATE_WPM} command_speech ${VOICE_SPEAK_COMMAND_RESPONSES_PARAM}"
...
echo "[mapping] tts: server ${VOICE_SERVER_TTS_ENABLED_PARAM}, connect_timeout ${VOICE_SERVER_TTS_CONNECT_TIMEOUT_SEC}s, timeout ${VOICE_SERVER_TTS_TIMEOUT_SEC}s, player ${VOICE_AUDIO_PLAYER}, system_sounds ${VOICE_SYSTEM_SOUND_DIR}, piper_model ${VOICE_PIPER_MODEL_PATH:-unset}"
echo "[mapping] command mux timeout: ${COMMAND_TIMEOUT}s"
if [[ "$VOICE_RECOGNITION_BACKEND" == "google" || "$VOICE_RECOGNITION_BACKEND" == "auto" || "$VOICE_GOOGLE_FALLBACK_ENABLED_PARAM" == "true" ]]; then
