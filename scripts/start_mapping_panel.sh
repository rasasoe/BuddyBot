...
CAMERA_DISCARD_BUFFERED_FRAMES="${BUDDYBOT_CAMERA_DISCARD_BUFFERED_FRAMES:-2}"
...
FOLLOW_BBOX_TIMEOUT="$(float_param_value "${BUDDYBOT_FOLLOW_BBOX_TIMEOUT:-0.6}")"
FOLLOW_MAX_SOURCE_AGE="$(float_param_value "${BUDDYBOT_FOLLOW_MAX_SOURCE_AGE:-0.8}")"
FOLLOW_HEIGHT_GAIN="$(float_param_value "${BUDDYBOT_FOLLOW_HEIGHT_GAIN:-0.0035}")"
FOLLOW_CENTER_GAIN="$(float_param_value "${BUDDYBOT_FOLLOW_CENTER_GAIN:-0.0012}")"
FOLLOW_TARGET_HEIGHT="$(float_param_value "${BUDDYBOT_FOLLOW_TARGET_HEIGHT:-0.50}")"
FOLLOW_MAX_LINEAR="$(float_param_value "${BUDDYBOT_FOLLOW_MAX_LINEAR:-0.16}")"
FOLLOW_MAX_ANGULAR="$(float_param_value "${BUDDYBOT_FOLLOW_MAX_ANGULAR:-0.10}")"
FOLLOW_MIN_LINEAR="$(float_param_value "${BUDDYBOT_FOLLOW_MIN_LINEAR:-0.10}")"
FOLLOW_COMMAND_RATE="$(float_param_value "${BUDDYBOT_FOLLOW_COMMAND_RATE:-10.0}")"
FOLLOW_LINEAR_ACCEL="$(float_param_value "${BUDDYBOT_FOLLOW_LINEAR_ACCEL:-0.12}")"
FOLLOW_ANGULAR_ACCEL="$(float_param_value "${BUDDYBOT_FOLLOW_ANGULAR_ACCEL:-0.12}")"
FOLLOW_BBOX_SMOOTHING_ALPHA="$(float_param_value "${BUDDYBOT_FOLLOW_BBOX_SMOOTHING_ALPHA:-0.25}")"
...
FOLLOW_HEIGHT_DEADZONE="${BUDDYBOT_FOLLOW_HEIGHT_DEADZONE:-30}"
...
echo "[mapping] camera pixel format: ${CAMERA_PIXEL_FORMAT} buffer ${CAMERA_BUFFER_SIZE} discard_buffered ${CAMERA_DISCARD_BUFFERED_FRAMES}"
echo "[mapping] detector profile: interval ${DETECT_INTERVAL}, confidence ${DETECT_CONFIDENCE}, hog confidence ${DETECT_HOG_CONFIDENCE}, hog resize ${DETECT_HOG_RESIZE_WIDTH}, hog fallback ${DETECT_ALLOW_HOG_FALLBACK_PARAM}, cascade fallback ${DETECT_ALLOW_CASCADE_FALLBACK_PARAM}"
echo "[mapping] follow profile: bbox_timeout ${FOLLOW_BBOX_TIMEOUT}s max_source_age ${FOLLOW_MAX_SOURCE_AGE}s max_linear ${FOLLOW_MAX_LINEAR} max_angular ${FOLLOW_MAX_ANGULAR} accel ${FOLLOW_LINEAR_ACCEL}/${FOLLOW_ANGULAR_ACCEL} allow_reverse ${FOLLOW_ALLOW_REVERSE_PARAM}"
...
if is_truthy "$DISABLE_CAMERA"; then
  ...
  if [[ -n "${CAMERA_DEVICE:-}" ]]; then
    ...
    start_node camera ros2 run buddybot_vision camera_node --ros-args -p width:="${CAMERA_WIDTH}" -p height:="${CAMERA_HEIGHT}" -p fps:="${CAMERA_FPS}" -p publish_rate:="${CAMERA_PUBLISH_RATE}" -p pixel_format:="${CAMERA_PIXEL_FORMAT}" -p buffer_size:="${CAMERA_BUFFER_SIZE}" -p discard_buffered_frames:="${CAMERA_DISCARD_BUFFERED_FRAMES}"
  fi
