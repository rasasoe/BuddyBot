...
FOLLOW_BBOX_TIMEOUT="$(float_param_value "${BUDDYBOT_FOLLOW_BBOX_TIMEOUT:-2.0}")"
FOLLOW_HEIGHT_GAIN="$(float_param_value "${BUDDYBOT_FOLLOW_HEIGHT_GAIN:-0.007}")"
FOLLOW_CENTER_GAIN="$(float_param_value "${BUDDYBOT_FOLLOW_CENTER_GAIN:-0.0035}")"
FOLLOW_TARGET_HEIGHT="$(float_param_value "${BUDDYBOT_FOLLOW_TARGET_HEIGHT:-0.50}")"
FOLLOW_MAX_LINEAR="$(float_param_value "${BUDDYBOT_FOLLOW_MAX_LINEAR:-0.28}")"
FOLLOW_MAX_ANGULAR="$(float_param_value "${BUDDYBOT_FOLLOW_MAX_ANGULAR:-0.28}")"
FOLLOW_MIN_LINEAR="$(float_param_value "${BUDDYBOT_FOLLOW_MIN_LINEAR:-0.14}")"
FOLLOW_COMMAND_RATE="$(float_param_value "${BUDDYBOT_FOLLOW_COMMAND_RATE:-10.0}")"
FOLLOW_LINEAR_ACCEL="$(float_param_value "${BUDDYBOT_FOLLOW_LINEAR_ACCEL:-0.25}")"
FOLLOW_ANGULAR_ACCEL="$(float_param_value "${BUDDYBOT_FOLLOW_ANGULAR_ACCEL:-0.35}")"
FOLLOW_BBOX_SMOOTHING_ALPHA="$(float_param_value "${BUDDYBOT_FOLLOW_BBOX_SMOOTHING_ALPHA:-0.35}")"
FOLLOW_BBOX_FILTER_RESET_SEC="$(float_param_value "${BUDDYBOT_FOLLOW_BBOX_FILTER_RESET_SEC:-0.9}")"
FOLLOW_CENTER_DEADZONE="${BUDDYBOT_FOLLOW_CENTER_DEADZONE:-30}"
FOLLOW_HEIGHT_DEADZONE="${BUDDYBOT_FOLLOW_HEIGHT_DEADZONE:-20}"
...
if is_truthy "$DISABLE_CAMERA"; then
  ...
  start_node follow_controller ros2 run buddybot_vision follow_controller_node --ros-args -p image_width:="${CAMERA_WIDTH}" -p image_height:="${CAMERA_HEIGHT}" -p bbox_timeout_sec:="${FOLLOW_BBOX_TIMEOUT}" -p height_gain:="${FOLLOW_HEIGHT_GAIN}" -p center_x_gain:="${FOLLOW_CENTER_GAIN}" -p target_height_ratio:="${FOLLOW_TARGET_HEIGHT}" -p max_linear_velocity:="${FOLLOW_MAX_LINEAR}" -p max_angular_velocity:="${FOLLOW_MAX_ANGULAR}" -p min_linear_velocity:="${FOLLOW_MIN_LINEAR}" -p deadzone_center:="${FOLLOW_CENTER_DEADZONE}" -p deadzone_height:="${FOLLOW_HEIGHT_DEADZONE}" -p command_rate_hz:="${FOLLOW_COMMAND_RATE}" -p linear_accel_limit:="${FOLLOW_LINEAR_ACCEL}" -p angular_accel_limit:="${FOLLOW_ANGULAR_ACCEL}" -p bbox_smoothing_alpha:="${FOLLOW_BBOX_SMOOTHING_ALPHA}" -p bbox_filter_reset_sec:="${FOLLOW_BBOX_FILTER_RESET_SEC}"
