...
FOLLOW_HEIGHT_GAIN="$(float_param_value "${BUDDYBOT_FOLLOW_HEIGHT_GAIN:-0.012}")"
FOLLOW_CENTER_GAIN="$(float_param_value "${BUDDYBOT_FOLLOW_CENTER_GAIN:-0.008}")"
FOLLOW_TARGET_HEIGHT="$(float_param_value "${BUDDYBOT_FOLLOW_TARGET_HEIGHT:-0.80}")"
FOLLOW_MAX_LINEAR="$(float_param_value "${BUDDYBOT_FOLLOW_MAX_LINEAR:-0.75}")"
FOLLOW_MAX_ANGULAR="$(float_param_value "${BUDDYBOT_FOLLOW_MAX_ANGULAR:-0.80}")"
FOLLOW_MIN_LINEAR="$(float_param_value "${BUDDYBOT_FOLLOW_MIN_LINEAR:-0.40}")"
...
set_speaker_volume() {
  ...
  if command -v wpctl >/dev/null 2>&1; then
    if wpctl set-volume @DEFAULT_AUDIO_SINK@ "${volume}%" >/dev/null 2>&1; then
      echo "[demo] Pi speaker volume set to ${volume}% via wpctl"
      return 0
    fi
  fi
  ...
  if command -v pactl >/dev/null 2>&1; then
    if pactl set-sink-volume @DEFAULT_SINK@ "${volume}%" >/dev/null 2>&1; then
      echo "[demo] Pi speaker volume set to ${volume}% via pactl"
      return 0
    fi
  fi
  ...
  if command -v amixer >/dev/null 2>&1; then
    if amixer -q sset Master "${volume}%" >/dev/null 2>&1; then
      echo "[demo] Pi speaker volume set to ${volume}% via amixer"
      return 0
    fi
  fi

  echo "[demo] warning: could not clamp Pi speaker volume to ${volume}%"
}
...
echo "[demo] Pi speaker volume target: ${SPEAKER_VOLUME_PERCENT}%"
...
if is_truthy "$DISABLE_CAMERA"; then
  ...
  if camera_streaming 8; then
    ...
    start_node detector ros2 run buddybot_vision detector_node --ros-args -p detection_interval:="${DETECT_INTERVAL}" -p confidence_threshold:="${DETECT_CONFIDENCE}" -p hog_resize_width:="${DETECT_HOG_RESIZE_WIDTH}" -p allow_hog_fallback:="${DETECT_ALLOW_HOG_FALLBACK_PARAM}" -p publish_debug_image:="${DETECT_PUBLISH_DEBUG_IMAGE_PARAM}"
    start_node follow_controller ros2 run buddybot_vision follow_controller_node --ros-args -p image_width:="${CAMERA_WIDTH}" -p image_height:="${CAMERA_HEIGHT}" -p bbox_timeout_sec:="${FOLLOW_BBOX_TIMEOUT}" -p height_gain:="${FOLLOW_HEIGHT_GAIN}" -p center_x_gain:="${FOLLOW_CENTER_GAIN}" -p target_height_ratio:="${FOLLOW_TARGET_HEIGHT}" -p max_linear_velocity:="${FOLLOW_MAX_LINEAR}" -p max_angular_velocity:="${FOLLOW_MAX_ANGULAR}" -p min_linear_velocity:="${FOLLOW_MIN_LINEAR}"
  else
