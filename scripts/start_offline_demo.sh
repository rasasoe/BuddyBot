...
if is_truthy "$DISABLE_CAMERA"; then
  ...
  if camera_streaming 8; then
    ...
    start_node follow_controller ros2 run buddybot_vision follow_controller_node --ros-args -p image_width:="${CAMERA_WIDTH}" -p image_height:="${CAMERA_HEIGHT}" -p bbox_timeout_sec:="${FOLLOW_BBOX_TIMEOUT}" -p height_gain:="${FOLLOW_HEIGHT_GAIN}" -p center_x_gain:="${FOLLOW_CENTER_GAIN}" -p target_height_ratio:="${FOLLOW_TARGET_HEIGHT}" -p max_linear_velocity:="${FOLLOW_MAX_LINEAR}" -p max_angular_velocity:="${FOLLOW_MAX_ANGULAR}"
  else
