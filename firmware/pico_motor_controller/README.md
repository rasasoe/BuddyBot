...
- 현재 현장 기준 기본값은 `left=+1`, `right=+1`, `back=+1` 입니다.
- 전진 명령에서 제자리 회전이 나면 먼저 이 값을 확인합니다.

## 측면 이동 보정
- `STRAFE_BACK_WHEEL_GAIN`은 측면 이동 때 뒤 바퀴에만 적용됩니다.
- 좌우 측면 이동이 서로 반대 방향 원호로 무너지면 뒤 바퀴의 실효 측면 출력이 부족한 패턴일 수 있습니다.
- 전진, 후진, 제자리 회전 기준선은 유지한 채 이 값만 소폭 조정합니다.
